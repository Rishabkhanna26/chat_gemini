...
export const hasBookingAccess = (valueOrUser) => {
  if (valueOrUser && typeof valueOrUser === 'object' && valueOrUser.admin_tier === 'super_admin') {
    return true;
  }
  return Boolean(valueOrUser?.booking_enabled);
};

export const hasAppointmentAccess = (valueOrUser) =>
  hasServiceAccess(valueOrUser) || hasBookingAccess(valueOrUser);
...
export const getAppointmentCapabilityLabel = (valueOrUser) => {
  const hasServices = hasServiceAccess(valueOrUser);
  const hasBookings = hasBookingAccess(valueOrUser);
  if (hasServices && hasBookings) return 'Services + Booking';
